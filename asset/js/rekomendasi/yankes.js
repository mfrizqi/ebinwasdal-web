$(document).ready( function () {
    oTable = $('#example5').DataTable( {
        "scrollX": true,
        "ajax": {
          "url": "../rekomendasi/yankes/getlist",
          "type": "GET"
        },
        "columns": [
            {"data": "id",
    ...
    function validasi(){
      ...
      var text ="";
      if ($('[name="jenis"]').val() == ""){
        text += "- Jenis yankes tidak boleh kosong <br> ";
        validasi = false;
      }
    ...
    function edit(id){
      ...
      $.ajax({
        url : "../rekomendasi/yankes/Edit/" + id,
        type: "GET",
        dataType: "JSON",
    ...
    function simpan(){
      if (validasi()){
      if (save_method == 1){
          var url = "../rekomendasi/yankes/Simpan";
      }else {
        var url = "../rekomendasi/yankes/Update";
      }
    $.ajax({
            url : url,
            type: "POST",
    ...
    function remove(id)
    {
      ...
      .one('click', '#delete', function(e) {
        $.post("../rekomendasi/yankes/hapus/" + id, function( data ) {
          console.log(id);
          refresh();
